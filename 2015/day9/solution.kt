...
data class Route(val loc1: String, val loc2: String, val distance: Int) {
    companion object {
        fun createFromString(strRep: String): Route {
            ...
            return regex.matchEntire(strRep)?.destructured?.let { (loc1, loc2, distance) -> Route(loc1, loc2, distance.toInt())} ?: throw IllegalArgumentException("Bad input $strRep")
        }
    }
}
...
fun readInput(): List<Route> {
    ...
    return input.map {
        value -> Route.createFromString(value)
    }.toList();
}
