...
def part2_original(connections: dict[str, set[str]]) -> int:
    ...
    while len(queue):
        ...
        for next in connections[cur]:
            if next.isupper() or next not in cur_path or (next != 'start' and not small_cave_twice and counter[next] < 2):
                queue.append((next, cur_path + [next], (small_cave_twice or (next.islower() and counter[next] == 1))))
...
def part2_skip(cave: str, visited_counts: dict[str, int]) -> bool:
    return cave.islower() and cave in visited_counts and any(cv.islower() and count == 2 for cv, count in visited_counts.items())
